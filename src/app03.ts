 ...
 * @param number 個数
 */
const createRandomMeshList = (
  number: number
): Array<{ mesh: THREE.Mesh; speed: number; axis: THREE.Vector3 }> => {
  const list: Array<{
    mesh: THREE.Mesh;
    speed: number;
    axis: THREE.Vector3;
...
const randomMesh = (): THREE.Mesh => {
  ...
  mesh.position.copy(
    new THREE.Vector3(20 - Math.random() * 40, 0, 20 - Math.random() * 40)
  );
...
const update = () => {
  for (const { mesh, axis, speed } of meshList) {
    mesh.applyQuaternion(
      new THREE.Quaternion().setFromAxisAngle(axis, speed * 0.2)
    );
  }
