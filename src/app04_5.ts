...
>> => {
  ...
  const meshListListList: Array<Array<
    Array<{
      mesh: THREE.Mesh;
      position: THREE.Vector3;
      rotation: Array<Rotation>;
  ...
  for (let x = 0; x < cubeNum; x++) {
    const meshListList: Array<Array<{
      mesh: THREE.Mesh;
      position: THREE.Vector3;
      rotation: Array<Rotation>;
    }>> = [];
      ...
      for (let z = 0; z < cubeNum; z++) {
        meshList.push({
          ...
          position: new THREE.Vector3(
            x * cubeSize.width - (cubeSize.width * (cubeNum - 1)) / 2,
            y * cubeSize.height - (cubeSize.height * (cubeNum - 1)) / 2,
            z * cubeSize.depth - (cubeSize.depth * (cubeNum - 1)) / 2
          ),
          rotation: []
        });
      }
...
const rotationDataToStandardName = (
  ...
  switch (rotationData.face) {
    case BaseFace.Front:
      ...
      return (
        "B" +
        turnDirectionToString(reverseTurnDirection(rotationData.turnDirection))
      );
    case BaseFace.Left:
      if (3 < size.width) {
        return null;
...
// ライトの設定
makeLightList(10).forEach((light): void => {
  scene.add(light);
        ...
        if (count < 90) {
          if (i === 0) {
            mesh.matrix = new THREE.Matrix4()
              .makeRotationAxis(axis.x, (count * Math.PI * 2) / 360)
              .multiply(new THREE.Matrix4().setPosition(position));
          } else {
          ...
          if (j === 0) {
            mesh.matrix = new THREE.Matrix4()
              .makeRotationAxis(axis.y, (count * Math.PI * 2) / 360)
              .multiply(new THREE.Matrix4().setPosition(position));
          } else {
            mesh.matrix = new THREE.Matrix4().multiply(
              new THREE.Matrix4().setPosition(position)
...
playStopButton.addEventListener("click", (): void => {
  isPlay = !isPlay;
  playStopButtonUpdate();
});
